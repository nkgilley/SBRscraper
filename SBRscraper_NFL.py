...
def main():
    ...
    if date.today().weekday() == 0:
        # get Monday
        todays_date = str(next_dow(date.today(),[0])).replace('-','')

    # if weekday is before or on thursday
    elif date.today().weekday() <= 2:
        # Get Thursday
        todays_date = str(next_dow(date.today(),[3])).replace('-','')

    # if weekday is before or on sunday
    elif date.today().weekday() <= 6:
        # Get Sunday & Monday
        todays_date = str(next_dow(date.today(),[6])).replace('-','')
    ...
    with open('NFL_Lines.csv', 'a') as f:
